...
// функции зашифровки и расшифровки
function festEncrypt (text, key) {
    ...
    return festProto(text, expandedKeys);
}

function festDecrypt (text, key) {
    key = hexToBytes(key);
    let expandedKeys = festExpandKey(key);
...
//Функции разделения расшифровка / зашифровка
function festPrepairPhrase(text, key, ifCyph) {
    ...
    if (ifCyph) {
        ...
        let final = "";
        textLen = text.length;
        for (let k = 0 ; k < textLen; k += 16) {
            let crypt_text = text.slice(k, k + 16);
            final += festEncrypt(crypt_text, key);
        }
        return final;
    } else {
        let final = "";
        ...
        for (let k = 0; k < textLen; k += 16) {
            let crypt_text = text.slice(k, k + 16);
            final += festDecrypt(crypt_text, key);
        }

        final = hexToBytes(final);
        final = new TextDecoder().decode(final);
        while (final[final.length - 1] == 'ф') {
            final = final.substring(0, final.length - 1);
        }

        final = textToPoint(final);
        return final;
    }
}
...
function encryptText() {
    ...
    if (check) {
        ...
        encryptedText = festPrepairPhrase(text, key, true);
    } else {
        encryptedText = bytesToHex(g(hexToBytes(text), hexToBytes(key)));
    }
...
function decryptText() {
    ...
    if (check) {
        decryptedText = festPrepairPhrase(text, key, false);
        decryptedText = textToPoint(decryptedText);
    } else {
        decryptedText = "Can`t decipher g";
    }
